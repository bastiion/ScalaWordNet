...
class WordNetTest {
  ...
  val wn = WordNet() // (new File("src/main/resources/wnconfig.xml"))
  ...
  def testSynsetLemmas(): Unit = {
    ...
    Assert.assertTrue(sl(1).getLemma == "auto")
  }
  ...
  def testSynsetLemma(): Unit = {
    ...
    sl match {
      case Some(x) =>
        ...
        Assert.assertTrue("automobile" == x.getLemma)
  ...
  def testHyponyms(): Unit = {
    ...
    val ambulance = typesOfMotorcar.head
  ...
  def testHypernymPaths(): Unit = {
    ...
    val paths0 = paths.head.map(ss => wn.format(ss))
